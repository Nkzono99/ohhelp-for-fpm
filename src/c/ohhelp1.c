...
static dint  assign_particles(dint npr, dint npt, struct S_node* ch, int incgp, int* nget);
...
static void  sched_comm(int toget, int rid, int tag, int reb, struct S_commsched_context* context);
static void  make_comm_count(int currmode, int level, int reb, int oldparent, int stats);
...
static void  stats_reduce_part(void* inarg, void* ioarg, int* len, MPI_Datatype* type);
static void  print_stats(struct S_statstotal* stotal, int cstep, int n);
static void  stats_reduce_time(void* inarg, void* ioarg, int* len, MPI_Datatype* type);
...
static void push_heap(int r, struct S_heap* heap, int greater) {
...
static void print_stats(struct S_statstotal* stotal, int step, int nstep) {
    int i;
    struct S_statstime* st = stotal->time;
    struct S_statspart* sp = stotal->part;
...
void vprint(char* format, ...) {
    ...
    if (verboseMode >= 3) { Vprint(format, "*Starting[%d] "); } else { Vprint_Norank(format, "*Starting "); }
}

void dprint(char* format, ...) {

    if (nOfNodes >= 1000) { Vprint(format, "#Debug[%04d] "); } else if (nOfNodes >= 100) { Vprint(format, "#Debug[%03d] "); } else if (nOfNodes >= 10) { Vprint(format, "#Debug[%02d] "); } else { Vprint(format, "#Debug[%d] "); }
}
